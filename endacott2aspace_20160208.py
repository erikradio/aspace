...
with open("Endacott_Society_Metadata.csv","rU") as csvFile:
	...
	# print(rep)
	# kill = requests.delete(aspace_url+"/repositories/2/resources/5",headers=headers).json()
	# print(kill)
	...
	#
	# end_chil_post=requests.post(aspace_url+"/repositories/2/resources/8/children",headers=headers,data=json.dumps(end_children)).json()
...
# get the digital objects stored in json
	adams=requests.get(aspace_url+"/repositories/2/archival_objects?id_set=384",headers=headers)
	# print(adams.text)
	thing=[]
	digital_ob=[]
	for x in range(3,14):
	...
	ao=requests.get(aspace_url+"/repositories/2/resources/8/tree",headers=headers).json()
